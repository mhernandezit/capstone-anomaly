...
class BGPCollector:
    ...
    async def collect_bgp_data(self) -> Dict:
        ...
        logger.info(
            f"Collected data from {data['devices_available']}/{data['devices_total']} devices"
        )
        logger.info(
            f"Found {len(data['neighbors'])} neighbors, {len(data['routes'])} routes, {len(data['logs'])} log entries"
        )
    ...
    async def start_monitoring(self, interval: int = 30):
        ...
        try:
            while self.running:
                ...
                # Log summary
                logger.info(
                    f"BGP Status: {data['devices_available']} devices, "
