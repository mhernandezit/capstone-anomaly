...
# Configure Streamlit
st.set_page_config(page_title="BGP Anomaly Detection", page_icon="🔍", layout="wide")
...
def render_anomaly_details(events):
    ...
    for i, anomaly in enumerate(anomalies):
        with st.expander(
            f"Alert #{len(anomalies)-i} - {anomaly['timestamp'].strftime('%H:%M:%S')}"
        ):
            ...
            with col2:
                ...
                st.write("- Error Messages: 3")
                st.write("- Warning Messages: 2")
                st.write("- Devices Affected: spine-01, tor-01")

            # Correlation analysis
            st.write("**Signal Correlation:**")
            st.write("- BGP Anomaly: ✓ True")
            st.write("- Syslog Anomaly: ✓ True")
            st.write("- Correlation Strength: 0.89")
            st.write("- Signal Agreement: ✓ True")
