...
st.markdown(
    '<h1 class="main-header">🌐 Network Anomaly Detection Dashboard</h1>', unsafe_allow_html=True
)
...
with col1:
    st.metric(
        ...
        value=len(
            [
                p
                for p in st.session_state.data_collector.peer_stats.values()
...
with col2:
    st.metric(
        ...
        value=len(
            [
                m
                for m in st.session_state.data_collector.bgp_data
...
with col3:
    st.metric(
        label="Anomalies Detected", value=len(st.session_state.data_collector.anomalies), delta=None
    )
...
with col4:
    st.metric(
        ...
        value=(
            len(st.session_state.data_collector.snmp_data)
...
if show_topology:
    ...
    if G.nodes():
        ...
        for node in G.nodes():
            ...
            node_y.append(y)
            node_text.append(
                f"{node}<br>Updates: {G.nodes[node]['updates']}<br>Notifications: {G.nodes[node]['notifications']}"
            )
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["BGP Activity", "SNMP Metrics", "Syslog Messages", "Peer Details", "Anomaly Analysis"]
)
...
with tab1:
    ...
    if st.session_state.data_collector.bgp_data:
        ...
        fig = px.line(
            timeline_data, x="time", y="count", color="type", title="BGP Messages Over Time"
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
...
with tab2:
    ...
    if st.session_state.data_collector.snmp_data:
        ...
        fig = px.bar(
            snmp_df, x="device", y="value", color="metric_type", title="SNMP Metrics by Device"
        )
        st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    if st.session_state.data_collector.syslog_data:
        ...
        recent_syslog = syslog_df.tail(20)[
            ["timestamp", "device", "severity", "scenario", "message"]
        ]
        ...
        fig = px.bar(x=scenario_counts.index, y=scenario_counts.values, title="Syslog Scenarios")
        st.plotly_chart(fig, use_container_width=True)
    else:
            ...
            peer_data.append(
                {
                    ...
                    "Status": (
                        "Active"
                        if stats["last_seen"] and (datetime.now() - stats["last_seen"]).seconds < 60
                        else "Inactive"
                    ),
                    "Updates": stats["updates"],
                    "Notifications": stats["notifications"],
                    "Keepalives": stats["keepalives"],
                    "Last Seen": (
                        stats["last_seen"].strftime("%H:%M:%S") if stats["last_seen"] else "Never"
                    ),
                }
            )
...
with tab4:
    ...
    if st.session_state.data_collector.anomalies:
        ...
        fig = px.bar(
            x=severity_counts.index, y=severity_counts.values, title="Anomaly Severity Distribution"
        )
        st.plotly_chart(fig, use_container_width=True)

        # Confidence distribution
        fig = px.histogram(
            anomaly_df, x="confidence", bins=20, title="Anomaly Detection Confidence Distribution"
        )
        st.plotly_chart(fig, use_container_width=True)

        # Recent anomalies table
        st.markdown("### Recent Anomalies")
        recent_anomalies = anomaly_df.tail(10)[
            ["timestamp", "type", "severity", "peer", "confidence", "description"]
        ]
