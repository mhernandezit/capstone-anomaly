...
class AnomalyDashboard:
    ...
    async def _handle_event(self, msg):
        ...
        try:
            ...
            if data.get("is_anomaly", False):
                ...
                self.alerts.append(
                    {"timestamp": data["timestamp"], "type": "anomaly", "data": data}
                )
        except Exception as e:
    ...
    def render_metrics_overview(self):
        ...
        with col1:
            if self.metrics["total_events"] > 0:
                anomaly_rate = (
                    self.metrics["anomalies_detected"] / self.metrics["total_events"]
                ) * 100
    ...
    def render_anomaly_details(self):
        ...
        for i, alert in enumerate(reversed(recent_alerts)):
            with st.expander(
                f"Alert #{len(recent_alerts)-i} - {datetime.fromtimestamp(alert['timestamp']).strftime('%H:%M:%S')}"
            ):
                data = alert["data"]
                ...
                with col1:
                    ...
                    st.write(
                        f"- Detected Series: {', '.join(data.get('bgp_analysis', {}).get('detected_series', []))}"
                    )
                    ...
                    if "bgp_analysis" in data and "series_results" in data["bgp_analysis"]:
                        ...
                        series_df = pd.DataFrame(
                            [
                                {
                                    "Series": k,
                                    "Discord Score": v.get("discord_score", 0),
                                    "Is Discord": v.get("is_discord", False),
                                }
                ...
                with col2:
                    ...
                    st.write(
                        f"- Error Messages: {syslog_analysis.get('severity_counts', {}).get('error', 0)}"
                    )
                    st.write(
                        f"- Warning Messages: {syslog_analysis.get('severity_counts', {}).get('warning', 0)}"
                    )
                    st.write(
                        f"- Devices Affected: {', '.join(syslog_analysis.get('device_counts', {}).keys())}"
                    )
                    ...
                    if "severity_counts" in syslog_analysis:
                        ...
                        fig = px.pie(
                            values=list(severity_data.values()), names=list(severity_data.keys())
                        )
                        st.plotly_chart(fig, use_container_width=True)
                ...
                st.write(
                    f"- Correlation Strength: {correlation.get('correlation_strength', 0):.2f}"
                )
    ...
    def render_syslog_monitor(self):
        ...
        for msg in reversed(recent_syslog):
            ...
            # scenario = msg.get('scenario', 'normal_operation')  # Unused variable
    ...
    def render_system_status(self):
        ...
        with col2:
            ...
            st.write(
                f"- Events/min: {self.metrics['total_events'] * 60 // max(int(time.time() - self.start_time), 1)}"
            )
    ...
    def render_alert_management(self):
        ...
        with col1:
            ...
            st.selectbox("Impact Filter", ["All", "EDGE_LOCAL", "NETWORK_IMPACTING"])
