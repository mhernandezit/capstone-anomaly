...
class DualSignalPipeline:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.bgp_aggregator = FeatureAggregator(bin_seconds=config.get("bin_seconds", 30))
    ...
    async def _process_dual_signal_bin(self, fb: FeatureBin):
        ...
        try:
            ...
            event = self._create_dual_signal_event(
                fb, mp_results, correlated_syslog, impact_results
            )
            ...
            if event["is_anomaly"]:
                logger.info(
                    f"Dual-signal anomaly detected: {event['impact_analysis']['impact']} - "
    ...
    def _correlate_syslog_messages(
        self, fb: FeatureBin, mp_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _enhanced_impact_scoring(
        self, fb: FeatureBin, mp_results: Dict[str, Any], syslog_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_correlation_strength(
        self, mp_results: Dict[str, Any], syslog_analysis: Dict[str, Any]
    ) -> float:
