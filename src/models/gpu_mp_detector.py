...
class GPUMPDetector:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(f"GPU acceleration enabled with {gpu_memory_limit} memory limit")
        ...
        self.min_buffer_size = window_bins + 10  # Need extra for proper MP calculation
        
        # Matrix Profile state
        self.mp_cache = {}
        self.last_discord_scores = {}
        
        logger.info(f"Initialized GPU MP detector with window_bins={window_bins}, "
    ...
    def _calculate_fallback_zscore(self, ts: np.ndarray) -> Dict[str, float]:
        ...
        if self.gpu_available:
            ...
            rolling_mean = cp.convolve(ts_gpu, cp.ones(self.window_bins) / self.window_bins, mode='valid')
            rolling_std = cp.sqrt(cp.convolve(ts_gpu**2, cp.ones(self.window_bins) / self.window_bins, mode='valid') - rolling_mean**2)
