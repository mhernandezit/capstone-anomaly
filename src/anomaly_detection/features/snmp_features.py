...
class SNMPFeatureExtractor:
    ...
    def add_snmp_metric(self, metric: Dict[str, Any]):
        ...
        # Convert simple format to structured format if needed
        normalized_metrics = self._normalize_metric_format(metric)

        # Add metric(s) to current bin
        for m in normalized_metrics:
            self.current_metrics.append(m)
            self.stats["metrics_processed"] += 1
            self.stats["devices_monitored"].add(m.get("device_id", "unknown"))

    def _normalize_metric_format(self, metric: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Convert simplified metric format to structured format.

        Handles both formats:
        1. Simple: {"device": "R1", "metrics": {"cpu_usage_percent": 25, "temperature": 50}}
        2. Structured: {"device_id": "R1", "metric_type": "system", "oid": "...", "value": 25}
        """
        # If already in structured format, return as-is
        if "metric_type" in metric and "oid" in metric:
            return [metric]

        # Convert simple format to structured
        normalized = []
        device_id = metric.get("device") or metric.get("device_id", "unknown")
        timestamp = metric.get("timestamp", datetime.now().timestamp())
        metrics_dict = metric.get("metrics", {})

        # Extract system metrics
        if "cpu_usage_percent" in metrics_dict:
            normalized.append(
                {
                    "device_id": device_id,
                    "timestamp": timestamp,
                    "metric_type": "system",
                    "oid": "cpu_usage_percent",
                    "value": metrics_dict["cpu_usage_percent"],
                }
            )

        if "memory_usage_percent" in metrics_dict:
            normalized.append(
                {
                    "device_id": device_id,
                    "timestamp": timestamp,
                    "metric_type": "system",
                    "oid": "memory_usage_percent",
                    "value": metrics_dict["memory_usage_percent"],
                }
            )

        # Extract environmental metrics
        if "temperature" in metrics_dict:
            normalized.append(
                {
                    "device_id": device_id,
                    "timestamp": timestamp,
                    ...
                    "oid": "temperature",
                    "value": metrics_dict["temperature"],
                }
            )

        if "fan_speed_rpm" in metrics_dict:
            normalized.append(
                {
                    "device_id": device_id,
                    "timestamp": timestamp,
                    "metric_type": "environmental",
                    "oid": "fan_speed_rpm",
                    "value": metrics_dict["fan_speed_rpm"],
                }
            )

        # Extract interface metrics
        if "if_errors" in metrics_dict:
            normalized.append(
                {
                    "device_id": device_id,
                    "timestamp": timestamp,
                    ...
                    "oid": "ifInErrors.1",
                    "value": metrics_dict["if_errors"],
                }
            )

        if "if_discards" in metrics_dict:
            normalized.append(
                {
                    "device_id": device_id,
                    "timestamp": timestamp,
                    "metric_type": "interface",
                    "oid": "ifInDiscards.1",
                    "value": metrics_dict["if_discards"],
                }
            )

        if "if_octets_in" in metrics_dict:
            normalized.append(
                {
                    "device_id": device_id,
                    "timestamp": timestamp,
                    "metric_type": "interface",
                    "oid": "ifInOctets.1",
                    "value": metrics_dict["if_octets_in"],
                }
            )

        if "if_octets_out" in metrics_dict:
            normalized.append(
                {
                    "device_id": device_id,
                    "timestamp": timestamp,
                    "metric_type": "interface",
                    "oid": "ifOutOctets.1",
                    "value": metrics_dict["if_octets_out"],
                }
            )

        # Return at least one metric if conversion produced nothing
        if not normalized:
            logger.warning(f"Could not extract any metrics from: {metric}")
            return []
    ...
    def _extract_bin_features(
        self, device_metrics: Dict[str, Dict[str, List]], bin_start: float, bin_end: float
    ) -> SNMPFeatureBin:
        ...
        for device_id, metric_types in device_metrics.items():
            ...
            if "environmental" in metric_types:
                device_temps, device_fans, device_power = self._extract_environmental_features(
                    metric_types["environmental"], device_id
                )
        ...
        interface_util_std = (
            np.std(interface_utilizations) if len(interface_utilizations) > 1 else 0.0
        )
    ...
    def _extract_interface_features(
        self, interface_metrics: List[Dict], device_id: str
    ...
    def _extract_system_features(
        self, system_metrics: List[Dict], device_id: str
    ...
    def _extract_environmental_features(
        self, env_metrics: List[Dict], device_id: str
    ...
    def _analyze_device_anomalies(
        self, metric_types: Dict[str, List], device_id: str
    ) -> Tuple[int, int]:
    ...
    def _calculate_environmental_stress(
        self, temperatures: List[float], fan_speeds: List[float], power_values: List[float]
    ) -> float:
    ...
    def _calculate_bgp_correlation(self, bin_start: float, bin_end: float) -> float:
        ...
        bgp_events_in_window = [
            event for event in self.bgp_events if bin_start <= event["timestamp"] <= bin_end
        ]
    ...
    def get_current_bin_features(self) -> Optional[SNMPFeatureBin]:
        ...
        return self._extract_bin_features(device_metrics, self.current_bin_start, current_time)
...
def create_unified_feature_vector(
    bgp_features: np.ndarray, syslog_features: np.ndarray, snmp_features: np.ndarray
) -> np.ndarray:
