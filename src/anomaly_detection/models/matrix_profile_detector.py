"""
Matrix Profile Detector for BGP Anomaly Detection

This module implements a Matrix Profile detector using the stumpy library.
It's designed for real-time BGP anomaly detection with topology-aware
...
class MatrixProfileDetector:
    """
    Streaming Matrix Profile detector for BGP anomaly detection.

    Features:
    - CPU-optimized implementation using stumpy
    - Streaming processing for real-time detection
    - Multiple time series support (withdrawals, announcements, churn)
    - Multiple Matrix Profile library support (stumpy, matrixprofile, or fallback)
    - Memory-efficient sliding window processing

    Note: This was previously named CPUMPDetector.
    """
    ...
    def __init__(
        ...
        # Initialize data buffers (need 2*window for stumpy + some extra)
        ...
        logger.info(
            f"Initialized CPU MP detector with {self.mp_library} library, "
    ...
    def _calculate_matrix_profile_stumpy(self, ts: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            # Stumpy requires len(ts) >= 2*m
            if len(ts_float) < 2 * self.window_bins:
                logger.debug(
                    f"Stumpy: insufficient data ({len(ts_float)} < {2*self.window_bins}), using fallback"
                )
                return self._calculate_fallback(ts)
            ...
            logger.debug(
                f"Stumpy: discord={discord_score:.2f}, threshold={self.discord_threshold}, anomaly={discord_score > self.discord_threshold}"
            )
            ...
            logger.error(f"Stumpy calculation error: {e}", exc_info=True)
            return self._calculate_fallback(ts)
...
def create_cpu_mp_detector(
    ...
    """
    Factory function to create a Matrix Profile detector.
    ...
    Returns:
        Configured MatrixProfileDetector instance
    """
