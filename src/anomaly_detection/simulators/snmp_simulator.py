...
class SNMPFailureSimulator:
    ...
    def inject_failure_scenario(
        self, scenario: HardwareFailureScenario, device: Dict[str, Any]
    ) -> List[SNMPMetric]:
    ...
    def _generate_optical_failure_metrics(
        self,
        device: Dict[str, Any],
    ...
    def _generate_cable_failure_metrics(
        self,
        device: Dict[str, Any],
        scenario: HardwareFailureScenario,
        severity: str,
    ...
    def _generate_memory_failure_metrics(
        self,
        device: Dict[str, Any],
        scenario: HardwareFailureScenario,
        severity: str,
    ...
    def _generate_thermal_failure_metrics(
        self,
        device: Dict[str, Any],
        scenario: HardwareFailureScenario,
        severity: str,
    ...
    def _generate_power_failure_metrics(
        self,
        device: Dict[str, Any],
        scenario: HardwareFailureScenario,
        severity: str,
    ...
    async def generate_metrics_cycle(self) -> List[SNMPMetric]:
        ...
        for device in self.config["devices"]:
            ...
            should_inject_failure = (
                self.config["failure_injection"]["enabled"]
                and random.random() < self.config["failure_injection"]["scenario_probability"]
                and len(self.active_scenarios)
                < self.config["failure_injection"]["max_concurrent_scenarios"]
            )
                ...
                for scenario_key in list(self.active_scenarios.keys()):
                    if device["id"] in scenario_key:
                        ...
                        device_metrics.extend(
                            self.inject_failure_scenario(scenario_state["scenario"], device)
                        )
    ...
    async def publish_metrics(self, metrics: List[SNMPMetric]):
        ...
        for metric in metrics:
            ...
            await self.nats_client.publish(self.config["nats_subject"], message.encode())
