...
class MultiModalPipeline:
    ...
    def __init__(self, config_path: str = "configs/multi_modal_config.yml"):
        ...
        self.bgp_aggregator = FeatureAggregator(
            bin_seconds=self.config.get('bin_seconds', 60)
        )
        self.snmp_extractor = SNMPFeatureExtractor(
            bin_seconds=self.config.get('bin_seconds', 60)
        )
        
        # ML models
        self.bgp_detector = MPDetector(
            window_bins=self.config.get('window_bins', 64)
        )
    ...
    async def _perform_multi_modal_analysis(self, feature_sources: Dict[str, Any], 
                                          current_time: float):
        ...
        elif 'bgp' in feature_sources and feature_sources['bgp']['anomaly_score'] > alert_thresholds.get('bgp_only', 0.7):
            should_alert = True
            alert_type = 'bgp'
        elif 'snmp' in feature_sources and feature_sources['snmp']['anomaly_score'] > alert_thresholds.get('hardware_only', 0.6):
            should_alert = True
            alert_type = 'hardware'
    ...
    async def _generate_multi_modal_alert(self, feature_sources: Dict[str, Any], 
                                        alert_type: str, confidence: float,
                                        timestamp: float) -> Optional[MultiModalAlert]:
        ...
        recommended_actions = self._generate_recommendations(failure_type, failure_subtype, affected_devices)
    ...
    def _localize_failure(self, feature_sources: Dict[str, Any], 
    ...
    def _calculate_temporal_correlation(self, feature_sources: Dict[str, Any], 
    ...
    def _generate_recommendations(self, failure_type: str, failure_subtype: str, 
                                affected_devices: List[str]) -> List[str]:
    ...
    def _determine_severity(self, confidence: float, device_count: int, 
    ...
    async def run_pipeline(self):
        ...
        try:
            ...
            while self.running:
                ...
                if self.stats['bgp_events_processed'] % 100 == 0 and self.stats['bgp_events_processed'] > 0:
