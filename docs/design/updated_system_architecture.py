...
def create_updated_system_architecture():
    ...
    dot = Digraph('Multi_Modal_System_Architecture', filename='docs/design/updated_system_architecture.gv', format='png')
    dot.attr(rankdir='TB', splines='ortho', size='10,12')
    dot.attr('node', fontname='Arial', fontsize='11', width='2.5', height='1')
    dot.attr('edge', fontname='Arial', fontsize='9')
    
    # Title
    dot.node('title', 'Multi-Modal Network Anomaly Detection System\nBGP + SNMP + Syslog Integration', 
    ...
    with dot.subgraph(name='cluster_data_sources') as sources:
        ...
        sources.node('bgp_source', 'BGP Updates\n(BMP Collector)', shape='box', style='filled', fillcolor='lightgreen')
        sources.node('snmp_source', 'SNMP Metrics\n(Hardware Health)', shape='box', style='filled', fillcolor='lightyellow')
        sources.node('syslog_source', 'Syslog Messages\n(Device Events)', shape='box', style='filled', fillcolor='lightcoral')
    
    # Message Bus
    dot.node('nats_bus', 'NATS Message Bus\n(Real-time Streaming)', 
             shape='ellipse', style='filled', fillcolor='lightsteelblue', fontsize='12')
    
    # Processing Layer
    with dot.subgraph(name='cluster_processing') as processing:
        processing.attr(label='ML Processing Pipeline', style='filled', fillcolor='mistyrose', fontsize='12')
        processing.node('feature_extraction', 'Multi-Modal\nFeature Extraction', shape='box', style='filled', fillcolor='wheat')
        processing.node('anomaly_detection', 'Anomaly Detection\n(Matrix Profile + ML)', shape='box', style='filled', fillcolor='orange')
        processing.node('impact_classification', 'Impact Classification\n(Topology-Aware)', shape='box', style='filled', fillcolor='yellow')
    ...
    with dot.subgraph(name='cluster_output') as output:
        ...
        output.node('dashboard', 'Streamlit Dashboard\n(Live Monitoring)', shape='box', style='filled', fillcolor='lightpink')
        output.node('alerts', 'Smart Alerts\n(Context-Aware)', shape='box', style='filled', fillcolor='lightcoral')
        output.node('operator', 'Network Operator\n(Decision Maker)', shape='box', style='filled', fillcolor='lightgreen')
    ...
    with dot.subgraph(name='cluster_metrics') as metrics:
        metrics.attr(label='Performance Targets', style='filled', fillcolor='honeydew', fontsize='11')
        metrics.node('latency', 'Detection Latency\n< 60 seconds', shape='note', style='filled', fillcolor='lightsteelblue')
        metrics.node('accuracy', 'False Positive Rate\n< 5%', shape='note', style='filled', fillcolor='lightsteelblue')
        metrics.node('coverage', 'Multi-Modal Coverage\nBGP + SNMP + Syslog', shape='note', style='filled', fillcolor='lightsteelblue')
