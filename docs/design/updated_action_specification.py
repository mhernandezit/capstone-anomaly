...
def create_updated_action_specification():
    ...
    dot = Digraph('Updated_Action_Specification', filename='docs/design/updated_action_specification.gv', format='png')
    dot.attr(rankdir='LR', splines='ortho', size='12,10')
    dot.attr('node', fontname='Arial', fontsize='10')
    dot.attr('edge', fontname='Arial', fontsize='8')
    
    # Title
    dot.node('title', 'Multi-Modal Network Anomaly Detection\nAction Specification', 
             shape='box', style='filled', fillcolor='lightblue', fontsize='14')
    ...
    with dot.subgraph(name='cluster_actors') as actors:
        ...
        actors.node('operator', 'Network Operator\n(Primary User)', shape='box', style='filled', fillcolor='lightgreen')
        actors.node('network', 'Network Devices\n(BGP, SNMP, Syslog)', shape='box', style='filled', fillcolor='lightcoral')
        actors.node('system', 'Detection System\n(ML Pipeline)', shape='box', style='filled', fillcolor='lightyellow')
    ...
    with dot.subgraph(name='cluster_use_cases') as uc:
        ...
        uc.node('uc_monitor', 'Monitor Multi-Modal\nNetwork Data', shape='ellipse', style='filled', fillcolor='wheat')
        uc.node('uc_detect', 'Detect Anomalies\n(ML Analysis)', shape='ellipse', style='filled', fillcolor='orange')
        uc.node('uc_classify', 'Classify Impact\n(Topology Analysis)', shape='ellipse', style='filled', fillcolor='yellow')
        uc.node('uc_respond', 'Respond to Failures\n(Alerts & Actions)', shape='ellipse', style='filled', fillcolor='lightpink')
    ...
    with dot.subgraph(name='cluster_actions') as actions:
        ...
        actions.node('action_collect', 'Collect Multi-Modal Data\n(BGP + SNMP + Syslog)', shape='note', style='filled', fillcolor='lightsteelblue')
        actions.node('action_extract', 'Extract Features\n(Multi-Source Correlation)', shape='note', style='filled', fillcolor='lightsteelblue')
        
        # ML Actions
        actions.node('action_analyze', 'Analyze with ML\n(Matrix Profile + Others)', shape='note', style='filled', fillcolor='lightsteelblue')
        actions.node('action_classify', 'Classify Impact Scope\n(Edge vs Network-wide)', shape='note', style='filled', fillcolor='lightsteelblue')
        
        # Response Actions
        actions.node('action_alert', 'Generate Smart Alerts\n(Context-Aware)', shape='note', style='filled', fillcolor='lightsteelblue')
        actions.node('action_visualize', 'Update Dashboard\n(Real-time Visualization)', shape='note', style='filled', fillcolor='lightsteelblue')
    ...
    with dot.subgraph(name='cluster_decisions') as decisions:
        ...
        decisions.node('decision_anomaly', 'Anomaly Detected?\n(ML Threshold)', shape='diamond', style='filled', fillcolor='orange')
        decisions.node('decision_impact', 'Impact Level?\n(Edge vs Network)', shape='diamond', style='filled', fillcolor='red')
        decisions.node('decision_response', 'Alert Required?\n(Severity Check)', shape='diamond', style='filled', fillcolor='purple')
    
    # Performance Constraints
    with dot.subgraph(name='cluster_constraints') as constraints:
        constraints.attr(label='Performance Constraints', style='filled', fillcolor='aliceblue', fontsize='12')
        constraints.node('constraint_speed', 'Real-time Processing\n< 60s Detection', shape='box', style='filled', fillcolor='lightsteelblue')
        constraints.node('constraint_accuracy', 'High Accuracy\n< 5% False Positives', shape='box', style='filled', fillcolor='lightsteelblue')
        constraints.node('constraint_coverage', 'Multi-Modal Coverage\nAll Data Sources', shape='box', style='filled', fillcolor='lightsteelblue')
