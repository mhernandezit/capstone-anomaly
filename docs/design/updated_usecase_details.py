...
def create_updated_usecase_details():
    ...
    with dot.subgraph(name='cluster_alternatives') as alt:
        ...
        alt.node('alt1', 'Alternative 1: Partial Data\nMissing BGP session\n→ Infer from SNMP/Syslog\n→ Reduced confidence alert', 
                shape='box', style='filled', fillcolor='lightyellow')
        
        alt.node('alt2', 'Alternative 2: Gradual Degradation\nSlow performance decline\n→ Trending analysis\n→ Predictive alerting', 
                shape='box', style='filled', fillcolor='lightyellow')
        
        alt.node('alt3', 'Alternative 3: False Positive\nNormal operational change\n→ Enhanced verification\n→ No alert sent', 
                shape='box', style='filled', fillcolor='lightyellow')
    
    # Exception Handling
    with dot.subgraph(name='cluster_exceptions') as exc:
        exc.attr(label='Exception Handling', style='filled', fillcolor='lavenderblush', fontsize='12')
        
        exc.node('exc1', 'Exception: Data Source Failure\nBGP collector down\n→ SNMP/Syslog only mode\n→ Degraded monitoring', 
                shape='box', style='filled', fillcolor='lightcoral')
        
        exc.node('exc2', 'Exception: ML Model Failure\nMatrix Profile computation error\n→ Fallback to thresholding\n→ Reduced accuracy', 
                shape='box', style='filled', fillcolor='lightcoral')
        
        exc.node('exc3', 'Exception: Dashboard Failure\nUI unavailable\n→ Direct alert notifications\n→ Emergency mode', 
                shape='box', style='filled', fillcolor='lightcoral')
    ...
    with dot.subgraph(name='cluster_metrics') as metrics:
        ...
        metrics.node('metric_detection', 'Detection Time:\n< 60 seconds', 
                    shape='note', style='filled', fillcolor='lightsteelblue')
        
        metrics.node('metric_accuracy', 'Accuracy Rate:\n> 95%', 
                    shape='note', style='filled', fillcolor='lightsteelblue')
        
        metrics.node('metric_coverage', 'Coverage:\nMulti-modal correlation', 
                    shape='note', style='filled', fillcolor='lightsteelblue')
