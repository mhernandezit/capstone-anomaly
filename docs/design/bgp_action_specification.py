...
def create_action_specification_uml():
    ...
    dot = Digraph(
        "BGP_Action_Specification", filename="docs/design/bgp_action_specification.gv", format="png"
    )
    ...
    with dot.subgraph(name="cluster_constraints") as constraints:
        constraints.attr(
            label="System Constraints & Invariants", style="filled", fillcolor="honeydew"
        )
        constraints.node(
            "constraint_latency",
            "Constraint: DetectionLatency\n< 60 seconds end-to-end",
            shape="box",
            style="filled",
        ...
        constraints.node(
            "constraint_accuracy",
            "Constraint: FalsePositiveRate\n< 5% for network impact events",
            shape="box",
            style="filled",
            fillcolor="lightsteelblue",
        )
        ...
        constraints.node(
            "invariant_data",
            "Invariant: DataIntegrity\nAll BGP updates processed in order",
            shape="box",
            style="filled",
            fillcolor="lightpink",
        )
        constraints.node(
            "invariant_session",
            "Invariant: SessionState\nBGP sessions monitored continuously",
            shape="box",
            style="filled",
            fillcolor="lightpink",
        )
...
def create_use_case_detail_diagram():
    ...
    detail = Digraph(
        "BGP_UseCase_Details", filename="docs/design/bgp_usecase_details.gv", format="png"
    )
    ...
    with detail.subgraph(name="cluster_failure_scenario") as scenario:
        scenario.attr(
            label="Use Case: Network Failure Detection & Response",
            style="filled",
            fillcolor="lightcyan",
        )
        ...
        scenario.node(
            "step5",
            "Step 5:\nDashboard shows localized\nfailure with affected prefixes",
            shape="box",
        )
