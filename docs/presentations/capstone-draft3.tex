...
Network devices continuously generate health information through two types of monitoring data. Hardware-based metrics report the physical condition of equipment through the Simple Network Management Protocol, which collects data such as processor utilization, memory consumption, temperature readings, and interface error counts (Cisco, 2006). Software state metrics track the operational status of routing protocols and other network control software. The Border Gateway Protocol is one such routing protocol that directs traffic across networks by allowing routers to advertise which destinations they can reach (Rekhter et al., 2006). When network conditions change due to equipment failures, cable breaks, or configuration errors, routers send BGP update messages to inform their neighbors about these changes. This project uses Isolation Forest to detect hardware failures from SNMP metrics and Matrix Profile to identify patterns indicating software failures in BGP routing data. Together, these two information sources provide complementary views of network device health: hardware metrics reveal physical equipment condition, while routing updates show how network software is functioning.
...
Figure \ref{fig:architecture} illustrates the dual-pipeline ML architecture showing how BGP and SNMP data flow through specialized detection algorithms, multi-modal fusion, and topology-aware triage to produce enriched alerts.
...
\begin{tikzpicture}[
    ...
    device/.style={circle, draw=black, thick, fill=cyan!15, minimum size=0.6cm, font=\tiny},
    spine/.style={circle, draw=black, thick, fill=blue!30, minimum size=0.7cm, font=\tiny},
    simulator/.style={rectangle, draw=black, very thick, fill=orange!20, text width=2.5cm, align=center, minimum height=1.2cm, font=\small},
    stream/.style={rectangle, draw=black, thick, fill=green!15, text width=2cm, align=center, minimum height=0.8cm, font=\small},
    arrow/.style={->, >=stealth, line width=1pt},
]

% LEFT: Topology Reference (compact)
\node[font=\normalsize\bfseries] (topo_label) {Network Hierarchy};
\node[spine, below=0.4cm of topo_label] (ref_s) {S};
\node[device, below left=0.5cm and -0.1cm of ref_s] (ref_t1) {T};
\node[device, below right=0.5cm and -0.1cm of ref_s] (ref_t2) {T};
\node[device, below=0.5cm of ref_t1] (ref_l1) {L};
\node[device, below=0.5cm of ref_t2] (ref_l2) {L};

\draw[line width=0.4pt] (ref_s) -- (ref_t1);
\draw[line width=0.4pt] (ref_s) -- (ref_t2);
\draw[line width=0.4pt] (ref_t1) -- (ref_l1);
\draw[line width=0.4pt] (ref_t2) -- (ref_l2);

\node[below=0.1cm of ref_l1, font=\scriptsize, text=gray, align=center] {4 Spine\\8 ToR\\8 Leaf};

% RIGHT: Simulation Flow
\node[simulator, right=3.5cm of ref_s] (bgp_sim) {BGP\\Simulator};
\node[simulator, below=1.2cm of bgp_sim] (snmp_sim) {SNMP\\Simulator};

% Box around both simulators
\node[draw=gray, dashed, thick, rounded corners, fit=(bgp_sim)(snmp_sim), inner sep=0.3cm] (sim_box) {};

\node[stream, right=2cm of bgp_sim] (bgp_stream) {Routing\\Updates};
\node[stream, right=2cm of snmp_sim] (snmp_stream) {Hardware\\Metrics};

% Topology config feeds both simulators (to the box)
\draw[arrow, dashed, gray, thick] ($(ref_s)!0.5!(ref_l1)$) -- node[above, font=\tiny, pos=0.4] {topology config} (sim_box.west);

% Simulators generate data streams
\draw[arrow, blue!70, thick] (bgp_sim) -- node[above, font=\tiny] {generates} (bgp_stream);
\draw[arrow, green!70!black, thick] (snmp_sim) -- node[above, font=\tiny] {generates} (snmp_stream);

% Streams flow to detection (indicated)
\node[right=0.3cm of $(bgp_stream)!0.5!(snmp_stream)$, font=\small, align=center] {$\rightarrow$\\ML\\Detection};

% Labels
\node[above=0.2cm of sim_box, font=\normalsize\bfseries, text=gray] {Data Generation};

\end{tikzpicture}
\caption{Test Environment Architecture: (Left) Network topology defines a 20-device hierarchy with 4 core spine routers, 8 distribution ToR switches, and 8 access leaf switches. (Right) Python-based simulators use topology configuration to generate realistic routing updates and hardware metrics as if from these devices. Generated data streams feed the ML detection system for controlled testing with 98\% baseline traffic and 2\% injected failures.}
\label{fig:lab}
\end{figure}
...
Hit@k metrics assess localization accuracy by checking if the correct failing component appears among the top k candidates ranked by system confidence (Järvelin \& Kekäläinen, 2002). Adapted from information retrieval and recommendation systems, they gauge the practical value of ranked predictions for operators investigating multiple causes. Hit@1 reflects perfect localization when the top prediction matches the actual failure, while Hit@3 and Hit@5 allow for correlated symptoms among components. High Hit@1 scores are ideal, as they quickly guide operators to the root cause, minimizing investigation time.
...
\end{axis}
\end{tikzpicture}
\caption{Detection Accuracy Metrics: Tuned system achieved perfect scores across all metrics (Precision=1.0, Recall=1.0, F1=1.0) for controlled evaluation scenarios with failure patterns represented in training data. Performance on novel failure patterns varies; see Limitations section.}
\label{fig:metrics}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Detection Delay Performance: Mean detection delay of 29.4 seconds, median 40.9 seconds, and P95 of 55.9 seconds, all below the 60-second target threshold for real-time operational response.}
\label{fig:delay}
\end{figure}

\subsection{Scenario-Specific Performance Analysis}

The extended evaluation suite, comprising 15 scenarios, offers deeper insights into system performance across various failure types. BGP route flapping generated 20 events each over about 16 seconds, testing the Matrix Profile algorithm's detection of periodic routing instability across spine routers, ToR switches, and leaf switches. These routing anomalies represent patterns identified in prior research as characteristic of network failures that require automated classification and response (Cheng et al., 2021).
...
\bibitem{skazin2021}
Skazin, A., Mironova, V., \& Gal, I. (2021).
Network anomaly detection methods: literature review.
\textit{E3S Web of Conferences}, 258, 05015.
Available at: \href{https://doi.org/10.1051/e3sconf/202125805015}{https://doi.org/10.1051/e3sconf/202125805015}
...
Machine learning approach for network monitoring and log data analysis.
\textit{International Journal of Computer Network and Information Security}, 11(7), 13--20.
Available at: \href{https://doi.org/10.5815/ijcnis.2019.07.02}{https://doi.org/10.5815/ijcnis.2019.07.02}
